...
class trinityBuilding extends MY_Controller {
	...
	public function tBAMIMSRequestVerification() {
		...
		if(($data['requestStatus'] == 'O') || ($data['requestStatus'] == 'A') || ($data['requestStatus'] == 'E') || ($data['requestStatus'] == 'S') || ($data['requestStatus'] == 'W') || ($data['requestStatus'] == 'C') || ($data['requestStatus'] == 'R')) {
			...
			if(!empty($results21)) {
				...
				$data['completedFlag'] = $results21[0]->completedFlag;
    ...
    public function tBAMIMSWorkerList() {
		...
		$data['completedFlag'] = $_GET["completedFlag"];
		...
		if(!empty($resultsA)) {
			...
			$data['startDateActual'] = $resultsA[0]->startDateActual;
			$data['completedDateActual'] = $resultsA[0]->completedDateActual;
			$data['receivedDate'] = $resultsA[0]->receivedDate;
			$data['daysActual'] = $this->_getWorkingDays($data['startDateActual'],$data['completedDateActual']);
