            ...
            <span class="ml-auto navbar-text">
				...
				<?php foreach($categoryset as $rowC) {?>
					...
					<?php if($rowC->grpcount > 0) {?>
						...
						<?php foreach($groupset as $rowG) {?>
							<?php if($rowC->categ == $rowG->categorySystemID) {?>    
								<a class="dropdown-item" selectSettings="<?php echo $rowG->sourceSystemID . "/" . $rowG->elementValueID; ?>"><?php echo $rowG->elementValueDescription; echo " (" . $_SESSION['sy'] . "-" . $_SESSION['sem'] . ")";?></a>                
							<?php } ?>
						<?php } ?>
						</div>
					<?php } ?>
				...
				<?php } ?>
                    ...
                    <li class="nav-item dropdown">   
						...
						<div class="dropdown-menu" aria-labelledby="navbarDropdown">
                            <a href="/trinity" class="dropdown-item" selectControl1="logout">Logout</a> 
